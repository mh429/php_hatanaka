...
<?php
// セッションがない場合はリダイレクト
if (empty($_SESSION['member_regist_byadmin'])) {
  header('Location: ./login.php');
  exit;
}
...
$password = $_SESSION['member_regist_byadmin']['password'] ?? '';
...
if (!empty($errors)) {
  // エラーをセッションに保存
  $_SESSION['member_regist_byadmin']['errors'] = $errors;
  // 登録画面に戻す
...
$sql_insert->execute([$name_sei, $name_mei, $gender, $pref_name, $address !== '' ? $address : NULL, $password_hash, $email]);
